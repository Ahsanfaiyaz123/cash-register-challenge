  ...
  describe '#initialize' do
    ...
    it 'creates a product with the correct attributes' do
     ...
     it 'throws an error for negative price' do
      expect { Product.new('GR1', 'Green Tea', -3.11) }.to raise_error(ArgumentError)
    end

    it 'throws an error for empty code' do
      expect { Product.new(nil, 'Green Tea', 3.11) }.to raise_error(ArgumentError)
    end

    it 'throws an error for empty name' do
      expect { Product.new('GR1', nil, 3.11) }.to raise_error(ArgumentError)
    end

    it 'throws an error for non-string code' do
      expect { Product.new(123, 'Green Tea', 3.11) }.to raise_error(ArgumentError)
    end
  end
